...
class AttentionLayer(D.Layer):
    def __init__(self, cfg, name=None):
        ...
        self.dropout = lambda i: L.dropout(i, dropout_prob=cfg['attention_probs_dropout_prob'],
...
class PositionwiseFeedForwardLayer(D.Layer):
    def __init__(self, cfg, name=None):
        ...
        self.dropout = lambda i: L.dropout(i, dropout_prob=prob,
                                           dropout_implementation="upscale_in_train", ) if self.training else i
...
class ErnieModel(D.Layer, PretrainedModel):
    def __init__(self, cfg, name=None):
        ...
        self.word_emb = D.Embedding([d_vocab, d_emb], param_attr=F.ParamAttr(name=append_name(name, 'word_embedding'),
                                                                             initializer=initializer))
        self.pos_emb = D.Embedding([d_pos, d_emb], param_attr=F.ParamAttr(name=append_name(name, 'pos_embedding'),
                                                                          initializer=initializer))
        self.sent_emb = D.Embedding([d_sent, d_emb], param_attr=F.ParamAttr(name=append_name(name, 'sent_embedding'),
                                                                            initializer=initializer))
...
class ErnieModelForPretraining(ErnieModel):
    ...
    def __init__(self, cfg, name=None):
        ...
        self.mlm = _build_linear(d_model, d_model, append_name(name, 'mask_lm_trans_fc'), initializer,
                                 act=cfg['hidden_act'])
