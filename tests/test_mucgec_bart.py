...
try:
    ...
    pytest.skip("MuCGEC dependencies (modelscope/addict) not available", allow_module_level=True)
...
@pytest.mark.heavy(repo='damo/nlp_bart_text-error-correction_chinese')
def test_mucgec_bart():
    m = MuCGECBartCorrector()
    sents = ["北京是中国的都。", "他说：”我最爱的运动是打蓝球“", "我每天大约喝5次水左右。", "今天，我非常开开心。"]
    res = m.correct_batch(sents)

    assert res[0]['target'] == '北京是中国的首都。'
    assert res[1]['target'] == '他说：“我最爱的运动是打篮球”'
    assert res[2]['target'] == '我每天大约喝5杯水左右。'
    assert res[3]['target'] == '今天，我非常开心。'
